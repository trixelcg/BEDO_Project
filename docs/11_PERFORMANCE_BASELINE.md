...
| 2026‑08‑21 | `phase2/security-remediation` | BEDO‑022 (deflector scope, weight removal) | 769 | 217 055 | 22 | 764.45 MB | 25.92 MB | 57.25 MB | 23.6 s | GPU counters identical to the baseline; fingerprint identical in all ten sections. The weight-removal proxies exist only while discs are on the pan, so an empty rig adds nothing to the scene. |
| 2026‑08‑22 | `phase2/security-remediation` | BEDO‑021 (drag & 2 s transfers) | 769 | 217 055 | 22 | 764.45 MB | 25.92 MB | 57.26 MB | 22.3 s | GPU counters identical to the baseline; 42 shader programs unchanged; fingerprint identical in all ten sections bar the JS chunk hash. **Scene object count 290 at rest, before *and* after a drag** — a carried or flying object is a clone raised on `pointerdown` and disposed when the flight lands, so it costs +4 `Object3D` / +2 meshes *only while in hand* (measured: 290/219 idle → 294/221 carrying → 290/219 installed). No new `useFrame`; the transfer is advanced inside the one that already runs, and no React render happens per pointer move. Mean rAF interval was, if anything, marginally lower while dragging (670 ms) than at rest (735 ms) — i.e. inside the software renderer's noise. `dist/` +10 KB of JS (1 240 895 → 1 251 320 B). |
| | | | | | | | | | | |
